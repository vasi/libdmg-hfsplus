...
Line in the sand:
  ...
  258b553fad5970e726db2bbac6d40131fa0ca8b9  *testa.dmg (glob)
  473866b04c74f32a2be03f064721f48441a880b0  *testb.dmg (glob)
...
Unfortunately, attributing builds does not update the cSum block, so we expect some differences here:
  ...
  473866b04c74f32a2be03f064721f48441a880b0  *testb.dmg (glob)
  e60433ef2b55768d9b5c60c42f011f3eb850d89c  *testa_updated.dmg (glob)
...
Note -- same same:
  ...
  258b553fad5970e726db2bbac6d40131fa0ca8b9  *testa.dmg (glob)
  258b553fad5970e726db2bbac6d40131fa0ca8b9  *testa_reverted.dmg (glob)
